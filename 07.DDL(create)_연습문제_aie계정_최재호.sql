...
-- employee_copy 테이블에 primay key제약 조건 추가
alter table employee_copy add primary key(emp_id);

-- employee 테이블에 dept_code에 외래키 제약조건 추가(부모테이블 department)
alter table employee add foreign key(dept_code) references department(dept_id);

-- employee 테이블에 job_code 외래키 제약조건 추가 (job 테이블)
...
RENT_MEM_NO NUMBER CONSTRAINT RENT_FK_MEM REFERENCES TB_MEMBER(MEMBER_NONUMBER) ON DELETE SET NULL,
